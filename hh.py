...
def unbox_block_info(block_info):
...
class _Solver:
    ...
    def run(self):
        ...
        solution = None
        client = CommuneClient(self.node_url)
        self.c_client = client

        block_number, block_and_key_hash_bytes, block_hash = unbox_block_info(self.block_info)

        nonce_start = random.randint(0, nonce_limit)
        nonce_end = nonce_start + self.update_interval

        while True:
            _, _ = _update_curr_block(self.block_info, self.c_client, self.key.public_key)
            block_number, block_and_key_hash_bytes, block_hash = unbox_block_info(self.block_info)
...
def solve_for_difficulty_fast(
    ...
    block_info = BlockInfo(-1, b"", None)
    key_bytes = key.public_key
    _, _ = _update_curr_block(block_info, c_client, key_bytes)

    solver = _Solver(
        0,
        1,
        update_interval,
        None,  # Remove solution_queue
        None,  # Remove stopEvent
        block_info,
        limit,
        key,
        node_url,
    ...
    return solver.run()  # Directly run the solver and return the result
if __name__ == "__main__":
    ...
    client.compose_call("faucet",module="Faucet", params=params, key=key)
